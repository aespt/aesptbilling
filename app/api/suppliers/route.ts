import { desc, eq, type InferInsertModel, sql } from 'drizzle-orm';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const countResult = await db.select({ count: sql<number>`count(*)` }).from(SuppliersTable);
...
export async function PUT(request: Request) {
  try {
    ...
    if (!body.id) {
      return NextResponse.json({ error: 'Supplier ID is required for updates' }, { status: 400 });
    }
    ...
    // Prepare update data
    const updateData: Partial<InferInsertModel<typeof SuppliersTable>> = {};
