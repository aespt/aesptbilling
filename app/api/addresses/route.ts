...
export async function GET() {
  try {
    ...
    const addresses = await db.select().from(AddressTable).orderBy(desc(AddressTable.created_at));
...
export async function POST(request: Request) {
  try {
    ...
    const newAddress = await db.insert(AddressTable).values(validatedData).returning();
