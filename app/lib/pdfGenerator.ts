...
export async function generateInvoicePDF(invoiceData: InvoiceData): Promise<string> {
  ...
  try {
        ...
        <div style="width: 75%; background-color: #ffffff; padding: 20px; display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="text-align: right; font-size: 12px">
            ${
              ...
              ${(primaryAddress as any).phone_no ? `Tel: ${(primaryAddress as any).phone_no}<br>` : ''}
              ${(primaryAddress as any).transaction_no ? `TRN NO: ${(primaryAddress as any).transaction_no}` : ''}
            `
                : ''
            }
          </div>
    ...
    console.log('HTML content created, now generating PDF...');
    ...
    try {
      ...
      return new Promise(async (resolve, reject) => {
        try {
          ...
          console.log('Generating PDF using html2canvas approach');
          ...
          console.log('PDF generated successfully with html2canvas, blob URL:', blobUrl);
          resolve(blobUrl);
        } catch (error) {
