...
interface PurchaseDetailsProps {
  ...
  setFormData: (formData: PurchaseFormData) => void;
  errors: FormErrors;
  setErrors: (errors: FormErrors) => void;
}

// Utility function to generate purchase number
const generatePurchaseNumber = () => {
  const lastNumber = parseInt(localStorage.getItem('lastPurchaseNumber') || '1000');
  const newNumber = lastNumber + 1;
  localStorage.setItem('lastPurchaseNumber', newNumber.toString());
  return `PO-${new Date().getFullYear()}${(new Date().getMonth() + 1)
    .toString()
...
export default function PurchaseDetails({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Generate purchase number if not already set
        if (!initialized.current && !formData.purchase_number) {
          setFormData({
            ...formData,
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <TextField
                        ...
                        helperText={errors.purchase_number}
                        size="small"
                      />
                    </div>
                  </div>
