-- Check if invoice_stage type exists, if not create it
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'invoice_stage') THEN
        CREATE TYPE "public"."invoice_stage" AS ENUM('SALE', 'PROFORMA', 'QUOTATION');
    END IF;
END $$;
...
-- Add the invoice_stage column if it doesn't exist
DO $$ 
BEGIN
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE "payment_details" ADD CONSTRAINT "payment_details_invoice_id_invoices_id_fk" 
